...
우리는 색상에 대한 정보를 가진 객체를 사용합니다. 각 색상의 정보는 RGB를 나타내기 위한 튜플 또는 HEX를 나타내기 위한 문자열로 표현됩니다.
...
const palette: Record<ColorName, ColorValue> = { // [!code ++]
  red: [255, 0, 0],
  green: '#00FF00',
  bule: [0, 0, 255]  // [!code error] // 에러 발생
}
```

오타를 발견했으니 이를 수정하고, `red` 프로퍼티의 RGB 값에서 `green` 값을 가져오려고 합니다.
...
const result = palette.red.at(1); // [!code error] // 에러 발생  // [!code focus]
```

하지만 오류가 발생했습니다. 이는 `palette.red` 값의 타입이 유니언 타입이기 때문에 발생하는 문제입니다.

TypeScript는 이 값의 타입이 `RGB`인지 `string`인지 알지 못하기 때문에 함부로 `at` 메서드를 사용하지 못합니다.
