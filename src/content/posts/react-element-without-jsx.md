...
`HTML`에 더 많은 상호작용을 추가할수록 코드의 양이 크게 증가하게 되었고, 이로 인해 클라이언트 측 상태 관리가 복잡해져 성능 문제가 발생하게 되었습니다. 이러한 문제를 해결하기 위해 등장한 `React`는 다음과 같은 특징 가집니다.
...
`JSX`를 사용하면 위에서 사용한 원시 `React` API를 `HTML`과 유사한 형태로 작성할 수 있습니다. 아래 코드의 `element` 및 `jsxElement` 변수는 모두 같은 결과를 반환합니다.
...
프로그래밍에서는 코드를 재사용하기 위해 함수를 사용합니다. 컴포넌트는 `React` 요소를 재사용하기 위한 함수입니다.

일반적으로 `React` 요소를 반환하지만, 기본적으로 함수이므로 문자열, `null`, 숫자 등 렌더링 가능한 다른 항목을 반환할 수도 있습니다.

다음과 같이 동일한 형태의 여러 `JSX`가 존재한다고 가정해봅시다.
